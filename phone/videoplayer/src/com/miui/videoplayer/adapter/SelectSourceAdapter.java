...
public class SelectSourceAdapter extends BaseGroupAdapter<MediaUrlInfo> {
	...
	public SelectSourceAdapter(Context context) {
		super(context);
		mBootInfoLoader.addListener(mLoadListener);
		mBootInfoLoader.load();
	}
	
	public void setData(OnlineLoader onlineLoader) {
		if(onlineLoader != null) {
			mOnlineLoader = onlineLoader;
			buildMediaUrlInfos(mOnlineLoader.getMediaUrlInfoList());
			refresh();
		}
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null) {
			View view = View.inflate(mContext, R.layout.vp_popup_select_source_item, null);
			viewHolder = new ViewHolder();
			viewHolder.title = (TextView) view.findViewById(R.id.vp_select_source_item);
		...
		if(mediaUrlInfo != null) {
			...
			if(mOnlineLoader != null) {
				BaseUri playingUri = mOnlineLoader.getPlayingUri();
				if(playingUri instanceof OnlineUri) {
					OnlineUri onlineUri = (OnlineUri) playingUri;
