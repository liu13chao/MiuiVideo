...
public class MediaRecommendSupply implements Observer {
	...
	private Object[] mediaRecommendations;
	...
	private void prepareData(Recommendation[] recommendations) {
		if(recommendations != null) {
			...
			mediaRecommendations = new Object[mediaRecommendationSize];
			for(int i = 0; i < mediaRecommendationSize; i++) {
				if(recommendations[i].midtype == MediaMidTypeValueDef.MID_TYPE_MEDIA) {
					mediaRecommendations[i] = recommendations[i].mediaInfo;
				} else if(recommendations[i].midtype == MediaMidTypeValueDef.MID_TYPE_PERSON){
					mediaRecommendations[i] = recommendations[i].personInfo;
				}
			}
		}
	}
